...
file_date   = '011326'; %FILLIN with the date string used in your file
file_idnums  = [1,5,6] ; %FILLIN with the LIST of numeric values of the file ids.
...
electrode_size = [1, 2, 3]; %FILLIN with the LIST of values for your spacings
...
all_spike_waveforms = {};
mean_p2p = [];
mean_waveform = zeros([num_files, 30]);
all_SNR = [];
...
for iF=1:num_files
    ...
    %FILLIN with relevant code
    mean_waveform(iF, :) = mean(spike_waveforms,1);
...
figure
plot(electrode_size, all_spike_rate) %FILLIN with relevant variable name
xlabel('Electrode size') %FILL IN label
...
for iF=1:num_files
    ...
    xlabel('Time in seconds') %FILL IN label
    ylabel('Voltage in mV') %FILL IN label, be sure to include units
...
figure
plot(time_waveforms, mean_waveform) %FILLIN with relevant variable name
xlabel('Time in seconds') %FILL IN label
ylabel('Voltage in mV') %FILL IN label, be sure to include units
legendEntries = {'Large', 'Medium', 'Small'};
legend(legendEntries)


%figure 4: mean peak-to-peak amplitude versus electrode electrode size (attach in your comprehension questions)
figure
plot(electrode_size, mean_p2p) %FILLIN with relevant variable name
xlabel('Elctrode size') %FILL IN label
ylabel('Mean peak-to-peak amplitude in mV') %FILL IN label, be sure to include units
...
for iF=1:num_files
    ...
    hist(all_p2p{iF}, histBins) %FILLIN with your data to make a histogram of peak-to-peak amplitudes observed across all spikes, using histBins
    xlabel('Peak-to-peak amplitude (mV)') %FILL IN label be sure to include units
    ylabel('Count') %FILL IN label, be sure to include units
    title(['E-size: ', num2str(electrode_size(iF))])
end %end loop through files
...
xlabel('Electrode size') %FILL IN label
ylabel('SNR') %FILL IN label, be sure to include units
